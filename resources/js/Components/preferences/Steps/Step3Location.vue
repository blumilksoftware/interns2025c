...
const locationOpenModel = computed({
  get: () => props.locationOpen,
  set: (v) => emit('update:locationOpen', v)
})

const locationText = computed(() => {
  const loc = props.form?.location
...
const effectiveRadius = computed(() => {
  ...
  return props.form?.radiusKm ?? null
})
</script>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="space-y-4">
      <div class="filter-item" data-filter-id="radius">
        ...
        <select v-model="form.radiusKm" @change="moveFilterById('radius')" class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:ring-indigo-500 focus:border-indigo-500" :disabled="!locationText || locationText.toLowerCase() === 'cała polska'">
