...
<template>
  <dialog ref="dialog" class="z-50 m-0 min-h-full min-w-full overflow-y-auto bg-transparent backdrop:bg-transparent">
    <div class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" scroll-region>
      ...
      >
        <div v-show="show" class="mb-6 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl transition-all sm:w-full sm:mx-auto relative z-[9999]" :class="maxWidthClass">
          <slot v-if="showSlot" />
        </div>
      </transition>
    </div>
