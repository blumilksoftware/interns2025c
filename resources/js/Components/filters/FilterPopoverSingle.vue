      ...
      <div v-if="isOpen" class="absolute z-50 mt-1 w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg p-2">
        <div class="max-h-60 overflow-auto">
          <label v-for="opt in options" :key="opt.value" class="checkbox-wrapper flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200 px-2 py-1 rounded hover:bg-gray-50 dark:hover:bg-gray-800">
            <input v-model="valueProxy" type="radio" :name="filterId" :value="opt.value" class="border-gray-300 dark:border-gray-700 transition-all duration-150 ease-in-out checked:scale-105">
            <span>{{ opt.labelKey ? t(opt.labelKey) : (opt.label || opt.value) }}</span>
          </label>
        </div>
        <div class="mt-2 flex justify-end gap-2">
          <button type="button" class="text-xs px-2 py-1 rounded-md border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-150 ease-in-out hover:-translate-y-0.5" @click="valueProxy = ''">{{ t('preferences.placeholders.any') }}</button>
