...
function generateNonOverlappingPawPrints(desiredPawCount, maxPlacementAttemptsPerPaw = 200) {
  const pawPrintList = []
  ...
  for (let pawIndex = 0; pawIndex < desiredPawCount; pawIndex++) {
    ...
    while (!isPlaced && attemptCount < maxPlacementAttemptsPerPaw) {
      ...
      if (candidatePaw.x < 4 || candidatePaw.x > 96 || candidatePaw.y < 4 || candidatePaw.y > 96) {
        continue
      }
      ...
      for (let existingIndex = 0; existingIndex < pawPrintList.length; existingIndex++) {
        const existingPaw = pawPrintList[existingIndex]
        const combinedRadius = candidateRadius + getEffectiveRadiusPercent(existingPaw)
        if (squaredDistance(candidatePaw, existingPaw) < combinedRadius * combinedRadius) {
  ...
  <div class="absolute inset-0 pointer-events-none overflow-hidden -z-10">
    ...
    >
      <svg
        ...
        viewBox="0 0 24 24"
        fill="currentColor"
        class="icon icon-tabler icons-tabler-filled icon-tabler-paw text-gray-400 opacity-75"
      >
