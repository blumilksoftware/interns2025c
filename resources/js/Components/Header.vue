  ...
  <header>
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      <div class="flex lg:flex-1">
        <a href="#" class="-m-1.5 p-1.5" @click="goToLandingPage">
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12">
        <NavLink href="#" @click="goToDashboard">{{ t('landing.navigation.pets') }}</NavLink>
        <NavLink href="#">{{ t('landing.navigation.about') }}</NavLink>
        <NavLink href="#">{{ t('landing.navigation.contact') }}</NavLink>
      </PopoverGroup>
      <div class="hidden lg:flex lg:flex-1 lg:justify-end">
        <NavLink v-if="$page.props.auth.user" href="/user/profile" class="text-sm/6 font-semibold text-gray-900">{{ t('navigation.profile') }}</NavLink>
        <NavLink v-else href="/login" class="text-sm/6 font-semibold text-gray-900">{{ t('landing.navigation.login') }} <span aria-hidden="true">&rarr;</span></NavLink>
      </div>
    </nav>
        ...
        <DialogPanel class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white p-4 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          ...
          <div class="mt-6 flow-root">
            <div class="-my-6 divide-y divide-gray-500/10">
              ...
              <div class="py-6">
                <NavLink v-if="$page.props.auth.user" href="/user/profile">{{ t('navigation.profile') }}</NavLink>
                <NavLink v-else href="/login">{{ t('landing.navigation.login') }}</NavLink>
              </div>
            </div>
          </div>
        </DialogPanel>
